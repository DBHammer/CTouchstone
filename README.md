...
1. `Poc Test`:When migrating applications to cloud databases or upgrading query execution engines, database performance and service stability are key factors in deciding whether to deploy. Although classical benchmarking (benchmarking) is widely used for performance evaluation, the results of benchmarking are always unsatisfactory due to the lack of features of new application. However, in most cases, using real application data in the testing environment is not possible due to the serious risk of sensitive information leakage. To effectively evaluate the database, we propose the Query-Aware Database Generator (QAG) to generate a synthetic application, including data and workloads, whose query execution costs are similar on the real and synthetic databases. Specifically, the performance similarity of each query is closely related to the similarity of the cardinality constraint, which is the size of the input and output on each operator along the query tree.
2. `Resource Planning`: When planning resources for an analytics engine, it is often difficult to predict how an ever-increasing workload will perform with current resources. However, with QAG, application developers can scale workloads on demand. Specifically, developers can scale up the size of each operator's inputs and outputs and use QAG to generate the expected size of the application.
3. `Optimization of query execution`: Since different query operators have different resource requirements, the optimization of one operator may encroach on the resources of other operators, which degrades the overall performance.QAG can help developers reproduce the production execution plan and observe the impact of the current optimization on real scenarios.
4. `Customized test scenarios`: In addition to the above functions, users can also customize test scenarios for testing by modifying the json file in the workload subfolder of the result file generated by query parsing (i.e., constraint chain information, the structure of the file will be mentioned later). 
## Steps to use
