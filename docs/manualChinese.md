...
配置文件为json格式，需要数据库连接信息和目录信息等信息。具体如上所示，其中数据库连接信息是连接目标数据库的连接配置信息，查询所在目录指的准备进行仿真的查询，而执行结果存放目录指的解析出来的数据库信息存放的目录。而数据库默认schema名称，这是由于postgresql的特性，必须指定属于的schema，这里填默认的public就好。
